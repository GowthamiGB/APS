...
```
```{r}
x=seq(138,202,0.1) #for plotting and not for bin value

y=(1/sqrt(2*pi*(8^2)))*exp(-(x-170)^2/(8^2))

plot(x,y) #plot using basic R
```
```{r}
x=seq(138,202,0.1) #for plotting and not for bin value
f_X=function(x){
...
p=ggplot(data=df)+
  ...
  geom_line(aes(x=x ,y=f_Y(x),color='Unkonown'))+
  scale_color_manual(name='students',values=c('Male Students'='red','Unkonown'='blue'))+
  labs(x='Height(cm)',y='Density',title='Continuous Random Variable')
p
```
```{r}
dnorm(190,170,8) #same result as f_X ->mean of 2nd graph,mean of first graph,mean=8
```
...
p=ggplot(data=df)+
  ...
  geom_line(aes(x=x ,y=dnorm(x,190,4),color='Unkonown'))+
  scale_color_manual(name='students',values=c('Male Students'='red','Unkonown'='blue'))+
  labs(x='Height(cm)',y='Density',title='Continuous Random Variable')
p
```
```{r}
#X~N(mu(mean)=170,sigma=8)
mu=170
sigma=8
x=165
dnorm(x,mu,sigma) #f_X(165)=how much probability density(probability/cm)we have at 165cm
pnorm(x,mu,sigma) #f_X(165)=Probability->P(X<=165cm)
pnorm(173,mu,sigma)-pnorm(169,mu,sigma) #area between 169 and 173
pnorm(x,mu,sigma,lower.tail=FALSE) #height greater than or equal to 165 -> normally lower.tail=TRUE by default which means takes the left value
```
...
mean(simulatedData_Male)
print('-------')
mean((simulatedData_Male-mean(simulatedData_Male))^2)  #mean of the squared deviation-> also called varience
...
```{r}
#relative frequency=data/1,00,000
#density=relative/bin width(0.2)
# Parameters
mu_Male = 170
sigma_Male = 8

# Bin width for density from relative frequency
delta = 2.0
