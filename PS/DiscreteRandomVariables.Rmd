...
s_software = c(5000, 10000) #base=0,premium=1
...
```{r}
sample(s_software,size=1,replace=TRUE,prob=p_software[1, ])
```
...
```{r}
sample(s_software,size=1,replace=TRUE,prob=p_software[2, ])
```

```{r}
sample(s_software,size=100,replace=TRUE,prob=p_software[2, ])
```
...
result = numeric(2)# Simulate whether sales happen in morning and afternoon appointments
  for (j in c(1:2)){
...
nsimulations = 1e5 #1e5=10 to the power 5 zeros
...
table(simulatedData)/nsimulations #frequency table
```
